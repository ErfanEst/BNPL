...
object FeatureStoreBuilder {
  ...
  private def buildExpr(tableAlias: String, col: String, default: String, index: Long): String = {

    logger.info(tableAlias, col, default, index)
    Thread.sleep(250)
  ...
  private def buildExprList(tableAlias: String, cols: Seq[String], defaults: Map[String, String], index: Long): Seq[String] = {
    cols.map { col =>
  ...
  def main(args: Array[String]): Unit = {
    ...
    val indices = index until index by -1
    ...
    logger.info(s"Program completed at: ${new java.util.Date(System.currentTimeMillis())}")
    logger.info(s"Total execution time: ${(System.currentTimeMillis() - startTime) / 1000.0} seconds")
  }
}